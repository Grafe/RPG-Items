...
public class Events implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent e) {
		...
		if ((rItem = ItemManager.toRPGItem(item)) != null) {
			...
			if (rItem.getMaxDurability() != -1) {
				int durability = meta.containsKey(RPGMetadata.DURABILITY) ? ((Number) meta
						.get(RPGMetadata.DURABILITY)).intValue() : rItem
						.getMaxDurability();
				durability--;
				if (durability <= 0) {
	...
	public void onProjectileHit(ProjectileHitEvent e) {
		...
		if (removeArrows.contains(entity.getEntityId())) {
			...
			RPGItem item = ItemManager.getItemById(rpgProjectiles.get(entity
					.getEntityId()));
			new BukkitRunnable() {
			...
			if (item == null)
				return;
			item.projectileHit((Player) ((Projectile) entity).getShooter(),
					(Projectile) entity);
		}
	}
	...
	public void onInventoryClose(InventoryCloseEvent e) {
		if (recipeWindows.containsKey(e.getPlayer().getName())) {
			...
			((Player) e.getPlayer()).sendMessage(ChatColor.AQUA
					+ "Recipe set for " + item.getName());
	...
	public void onInventoryClick(InventoryClickEvent e) {
		if (useLocaleInv && e.getView() instanceof LocaleInventory) {
			...
			InventoryClickEvent clickEvent = new InventoryClickEvent(
					inv.getView(), e.getSlotType(), e.getRawSlot(),
			...
			if (clickEvent.isCancelled()) {
				...
				case ALLOW:
					...
					inv.getView().setItem(e.getRawSlot(),
	...
	public void onInventoryOpen(final InventoryOpenEvent e) {
		...
		if (e.getInventory().getType() != InventoryType.CHEST || !useLocaleInv) {
			...
			LocaleInventory localeInv = new LocaleInventory(
					(Player) e.getPlayer(), e.getView());
			e.setCancelled(true);
	...
	private int playerDamager(EntityDamageByEntityEvent e, int damage) {
		...
		if (!WorldGuard.canPvP(player.getLocation()) && !rItem.ignoreWorldGuard)
			return damage;
		damage = rItem.getDamageMin() != rItem.getDamageMax() ? (rItem
				.getDamageMin() + random.nextInt(rItem.getDamageMax()
				- rItem.getDamageMin())) : rItem.getDamageMin();
	...
	private int projectileDamager(EntityDamageByEntityEvent e, int damage) {
		...
		if (rpgProjectiles.contains(entity.getEntityId())) {
			RPGItem rItem = ItemManager.getItemById(rpgProjectiles.get(entity
					.getEntityId()));
			if (rItem == null)
				return damage;
			damage = rItem.getDamageMin() != rItem.getDamageMax() ? (rItem
	...
	private int playerHit(EntityDamageByEntityEvent e, int damage) {
		...
		for (int i = 0; i < armour.length; i++) {
			...
			if (pRItem.getArmour() > 0) {
				damage -= Math.round(((double) damage)
						* (((double) pRItem.getArmour()) / 100d));
			}
			RPGMetadata meta = RPGItem.getMetadata(pArmour);
			if (pRItem.getMaxDurability() != -1) {
				int durability = meta.containsKey(RPGMetadata.DURABILITY) ? ((Number) meta
						.get(RPGMetadata.DURABILITY)).intValue() : pRItem
						.getMaxDurability();
				durability--;
				if (durability <= 0) {
					armour[i] = null;
