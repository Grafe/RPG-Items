...
public class PowerIce extends Power implements PowerRightClick {
	...
	public void rightClick(final Player player) {
		...
		if (cooldown <= System.currentTimeMillis() / 50) {
			...
			final FallingBlock block = player.getWorld()
					.spawnFallingBlock(player.getLocation().add(0, 1.8, 0),
							Material.ICE, (byte) 0);
				...
				public void run() {
					...
					loop: for (int x = -1; x < 2; x++) {
						for (int y = -1; y < 2; y++) {
							for (int z = -1; z < 2; z++) {
								...
								if (world.getBlockTypeIdAt(loc) != Material.AIR
										.getId()) {
									Block b = world.getBlockAt(loc);
									if (b.getType().isSolid()) {
										if (checkBlock(loc.getBlockX(),
												loc.getBlockY(),
												loc.getBlockZ(), 1, 1, 1,
												bLoc.getX() - 0.5d,
												bLoc.getY() - 0.5d,
												bLoc.getZ() - 0.5d, 1, 1, 1)) {
											hit = true;
											break loop;
										}
									}
					...
					if (block.isDead() || hit) {
						...
						for (int x = -1; x < 2; x++) {
							for (int y = -1; y < 3; y++) {
								for (int z = -1; z < 2; z++) {
									Location loc = block.getLocation().add(x,
											y, z);
									Block b = world.getBlockAt(loc);
									if (!b.getType().isSolid()) {
										changedBlocks.put(b.getLocation(),
												b.getTypeId()
														| (b.getData() << 16));
						...
						(new BukkitRunnable() {
							...
							public void run() {
								for (int i = 0; i < 4; i++) {
									...
									int index = random.nextInt(changedBlocks
											.size());
									long data = changedBlocks.values()[index];
									Location position = (Location) changedBlocks
											.keys()[index];
									changedBlocks.remove(position);
									Block c = position.getBlock();
									position.getWorld().playEffect(position,
											Effect.STEP_SOUND, c.getTypeId());
	...
	private boolean checkBlock(double x1, double y1, double z1, double w1,
			double h1, double d1, double x2, double y2, double z2, double w2,
			double h2, double d2) {
		if (x1 + w1 < x2)
			return false;
		if (x2 + w2 < x1)
	...
	public String displayText(String locale) {
		return ChatColor.GREEN
				+ String.format(Locale.get("power.ice", locale),
						(double) cooldownTime / 20d);
	}
