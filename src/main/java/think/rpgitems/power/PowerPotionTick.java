...
public class PowerPotionTick extends Power implements PowerTick {
	...
	public void tick(Player player) {
		...
		for (PotionEffect potionEffect : player.getActivePotionEffects()) {
			if (potionEffect.getType().equals(effect)
					&& potionEffect.getDuration() == 25) {
				player.addPotionEffect(new PotionEffect(effect, 30, amplifier,
						true), true);
				hasEffect = true;
				break;
			}
		...
		if (!hasEffect) {
			player.addPotionEffect(
					new PotionEffect(effect, 30, amplifier, true), true);
		}
	...
	public String displayText(String locale) {
		return ChatColor.GREEN
				+ String.format(Locale.get("power.potiontick", locale), effect
