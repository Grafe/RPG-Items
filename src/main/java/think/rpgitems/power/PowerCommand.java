...
public class PowerCommand extends Power implements PowerRightClick,
		PowerLeftClick {
	...
	public void rightClick(Player player) {
		if (isRight) {
			long cooldown;
			RPGValue value = RPGValue.get(player, item, "command." + command
					+ ".cooldown");
			if (value == null) {
				cooldown = System.currentTimeMillis() / 50;
				value = new RPGValue(player, item, "command." + command
						+ ".cooldown", cooldown);
			} else {
			...
			if (cooldown <= System.currentTimeMillis() / 50) {
				...
				player.sendMessage(ChatColor.AQUA
						+ String.format(Locale.get("message.cooldown",
								Locale.getPlayerLocale(player)),
								((double) (cooldown - System
										.currentTimeMillis() / 50)) / 20d));
			}
		}
	}
