...
/**
 * RPGMetadata is a (bad) way of tagging extra data onto an item without it
 * showing up on the item itself. This uses a similar system to Notch's entity
 * metadata system. The data will be encoded in minecraft colour codes which
 * makes this really inefficient because 1 byte -> 2 bytes(Hex) -> 4 bytes(
 * Colour codes). RPGMetadatais prefixed with CAFE to allow it to be easily
 * detected.
 * 
 */
public class RPGMetadata extends TIntObjectHashMap<Object> {

	private final static String METADATA_PREFIX = ChatColor.COLOR_CHAR + "c"
			+ ChatColor.COLOR_CHAR + "a" + ChatColor.COLOR_CHAR + "f"
	...
	public static RPGMetadata parseLoreline(String lore) {
		...
		String lenStr = lore.substring(pos + METADATA_PREFIX.length(), pos
				+ METADATA_PREFIX.length() + 8);
		int length = parseShort(lenStr, 0);
		String data = lore.substring(pos + METADATA_PREFIX.length() + 8, pos
				+ METADATA_PREFIX.length() + 8 + length);
		int off = 0;
		while (off < length) {
	...
	private static int parseByte(String bStr, int off) {
		return Integer.parseInt(
				"" + bStr.charAt(off + 1) + bStr.charAt(off + 3), 16);
	}
	 ...
	 * insertShort(out, offset, s >> 16); insertShort(out, offset + 8, s &
	 * 0xFFFF); }
	 */
}
